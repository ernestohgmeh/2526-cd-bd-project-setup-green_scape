...
class PlantDocumentSystem:
    def __init__(self, db_config, base_path='/greenscape_docs'):
        ...
        self.base_docs_path.mkdir(exist_ok=True)
        self.create_indexes()
    ...
    def get_plant_directory(self, plant_id):
        plant_dir = self.base_docs_path / str(plant_id)
    ...
    def insert_document_metadata(self, plant_id, tipo_documento, filename, filepath, 
                                 mime_type, file_size, is_principal=False, parent_doc_id=None):
        connecion = self.get_db_connection()
        ...
        cursor.execute(query, (
            plant_id, tipo_documento, filename, str(filepath), 
            mime_type, file_size, is_principal, parent_doc_id
        ))
    ...
    def create_main_document(self, plant_id, file_content, filename, 
                            plant_data=None, mime_type=None):
        json_mime_types = [
            'application/json',
        ...
        filepath = self.save_document_file(plant_id, file_content, filename, is_principal=True)
    ...
    def create_secondary_document(self, plant_id, tipo_documento, file_content, filename,
                                 mime_type=None, parent_doc_id=None):
        if not mime_type:
            mime_type = self.detect_mime_type(filename)
        
        filepath = self.save_document_file(plant_id, file_content, filename, is_principal=False)
        file_size = os.path.getsize(filepath)
