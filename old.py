...
def query_selector_section():
    ...
    selected_query = st.selectbox(
        "Selecciona una consulta:", list(queries.keys()), key="query_selector"
    )
    ...
    with col2:
        export_button = st.button("📤 Exportar Resultados", use_container_width=True)
    with col3:
    ...
    if execute_button:
        with st.spinner("Ejecutando consulta..."):
            ...
            st.success(
                f"✅ Consulta ejecutada correctamente. Resultados: {len(placeholder_data)} filas"
            )
    ...
    if st.session_state.query_results is not None:
        ...
        with col_stats3:
            numeric_cols = len(
                st.session_state.query_results.select_dtypes(
...
def user_analysis_section():
    ...
    st.markdown(
        "Utiliza el procedimiento almacenado `sp_analisis_usuario` para obtener métricas detalladas de actividad."
    )
    ...
    with col_left:
        ...
        selected_user = st.selectbox(
            "Seleccionar Usuario:",
            users_data.apply(
                lambda x: f"{x['ID']} - {x['Nombre']} ({x['Email']})", axis=1
            ),
        )
        ...
        analyze_button = st.button(
            "🔍 Ejecutar Análisis de Usuario", use_container_width=True, type="primary"
        )
        ...
        if analyze_button:
            ...
            with st.spinner("Analizando actividad del usuario..."):
                ...
                metrics = [
                    ...
                    (
                        "Actividad Total",
                        results["TotalPublicaciones"]
                        + results["TotalComentarios"]
                        + results["TotalContribuciones"],
                        "📊",
                    ),
                ]
...
def conversation_management_section():
    ...
    with tab1:
        ...
        with col1:
            ...
            selected_publication = st.selectbox(
                "Publicación:",
                publications.apply(
                    lambda x: f"ID {x['ID']}: {x['Texto'][:50]}...", axis=1
                ),
            )
        ...
        with col2:
            ...
            parent_comment = st.selectbox(
                ...
                comment_threads.apply(
                    lambda x: f"{x['ID']}: {x['Texto']}"
                    if pd.notna(x["ID"])
        ...
        comment_text = st.text_area(
            "Tu comentario:", placeholder="Escribe tu comentario aquí...", height=150
        )
    ...
    with tab2:
        ...
        selected_conversation = st.selectbox(
            ...
            initial_comments.apply(
                lambda x: f"💬 {x['Texto'][:60]}... ({x['Respuestas']} respuestas)",
                axis=1,
            ),
        )
...
def document_explorer_section():
    ...
    st.markdown(
        "Visualiza todos los documentos asociados a una planta de forma organizada y jerárquica."
    )
    ...
    selected_plant = st.selectbox(
        "Seleccionar Planta:",
        plants_data.apply(
            lambda x: f"{x['ID']}: {x['Nombre']} ({x['Categoría']}) - {x['Documentos']} documentos",
            axis=1,
        ),
    )
    ...
    if st.session_state.selected_plant:
        ...
        tab_tree, tab_list, tab_details = st.tabs(
            ["🌳 Vista de Árbol", "📋 Lista de Documentos", "🔍 Detalles"]
        )
        ...
        with tab_details:
            ...
            with col2:
                st.metric(
                    ...
                    len(
                        [
                            d
                            for d in flatten_documents(documents)
                            if d["type"] != "principal"
                        ]
                    ),
                )
            ...
            doc_types = pd.DataFrame(flatten_documents(documents))[
                "type"
            ].value_counts()
            fig = px.pie(
...
def price_manager_section():
    ...
    with col_left:
        ...
        selected_product = st.selectbox(
            "Producto:",
            products.apply(
                lambda x: f"{x['ID']}: {x['Nombre']} - ${x['Precio Actual']}", axis=1
            ),
        )
        ...
        with st.form("price_update_form"):
            ...
            if submitted:
                if new_price != product_info["Precio Actual"]:
                    ...
                    st.success(
                        f"✅ Precio actualizado de ${product_info['Precio Actual']} a ${new_price}"
                    )
                    ...
                    audit_record = {
                        ...
                        "porcentaje_cambio": (
                            (new_price - product_info["Precio Actual"])
                            / product_info["Precio Actual"]
                        )
                        * 100,
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "Precio Anterior": st.column_config.NumberColumn(
                    "Precio Anterior", format="$%.2f"
                ),
                "Precio Nuevo": st.column_config.NumberColumn(
                    "Precio Nuevo", format="$%.2f"
                ),
...
def show_dashboard():
    ...
    with col_chart1:
        ...
        for i, (key, values) in enumerate(activity_data.items()):
            fig.add_trace(go.Bar(x=months, y=values, name=key, marker_color=colors[i]))
...
def show_configuration():
        ...
        with st.form("db_config_form"):
            ...
            with col2:
                ...
                db_port = st.number_input(
                    "Puerto", value=3306, min_value=1, max_value=65535
                )
    ...
    with tab3:
        ...
        session_timeout = st.slider(
            "Tiempo de expiración de sesión (minutos)", 15, 240, 60
        )
